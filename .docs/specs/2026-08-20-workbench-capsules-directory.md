...
9. 页眉视觉上一行：能看到品牌、`n / m 个差异`、新增/删除/修改、上一个/下一个、导出簇；**没有**字段/配置项两行；页眉不再渲染 `.ui-diff-field-row`。
10. 目录展开时顶栏有「n 个字段变化 · 涉及 m 个配置项」（可解析且有变化时）；组头与字段带 `L{n}`。
11. 默认目录展开；点手柄收起/展开有宽度动画（减少动效时瞬时）；收起后对照变宽，再点能打开。手柄常驻于缩略轨与目录之间，不在页眉再放一份。
12. 点组头/字段滚到对应行并选中；行号与 gutter 一致（删除→参考，其余→目标；不显示左右两个行号）。
13. 缩略轨快速上下拖再松开：当前视口差异高亮仍在；随后点目录仍跳转。
14. 上一个/下一个仍按文本块步进并绕回；可见文案为「上一个 / 下一个」，`aria-label` 仍为「上一个差异 / 下一个差异」。
15. `pnpm lint` / `pnpm test:run` / `pnpm build` 通过。
