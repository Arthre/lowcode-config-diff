...
- 页眉工具：差异徽章（无差异 / 当前序号）、上一个差异 / 下一个差异、查找（Ctrl+F）、复制、导出菜单（含压缩）；`ThemeToggle` 与「本地处理」悬停说明并列于首行右侧
...
| `src/views/HomeView.vue`                    | 工作台：满高 `TwoWayMergeEditor`；页眉主题切换、差异徽章、块导航、查找、复制、导出/压缩导出                                       |
| `src/components/TwoWayMergeEditor.vue`      | 可编辑 MergeView：栏头导入/粘贴/清空；轻量空态；查找条在编辑器外；右侧冲突点缩略轨；中间仅 `→`；按行 diff；滚动只更新视口锚点     |
...
- `TwoWayMergeEditor`：左参考配置、右目标配置；栏头导入/粘贴全文/清空，整栏拖入走 `importSide`（合法则格式化一次）；空栏轻量提示；编辑器内 Ctrl+V 仍是光标插入、不格式化；`→` 把参考块写入目标配置（`←` 暂时下线）；按行 diff；冲突点缩略轨（双侧都空时隐藏，滚动不重算冲突带）；Ctrl+F 打开编辑器外查找条；上一个差异/下一个差异按视口与块像素带重叠锚点步进并绕回（滚动不重测每块 `lineBlockAt`）；页眉当前块随视口滚动
...
- 计划（未归档）：[块导航滚动性能](../plans/2026-08-19-chunk-nav-scroll-perf.md)
