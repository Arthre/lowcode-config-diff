...
- 页眉工具：差异徽章（无差异 / 当前序号；有差异时徽章下显示新增 / 删除 / 修改构成副行）、上一个差异 / 下一个差异、查找（Ctrl+F）、复制、导出菜单（含压缩）；`ThemeToggle` 与「本地处理」悬停说明并列于首行右侧
...
| `src/views/HomeView.vue`                    | 工作台：满高 `TwoWayMergeEditor`；页眉主题切换、差异徽章与类型构成副行、块导航、查找、复制、导出/压缩导出                                                                |
| `src/components/TwoWayMergeEditor.vue`      | 可编辑 MergeView：栏头导入/粘贴/清空；轻量空态；查找条在编辑器外；右侧差异块目录 + 冲突点缩略轨；中间仅 `→`（按类型着色，当前锚点块可辨）；按行 diff；滚动只更新视口锚点 |
| `src/components/ChunkJumpList.vue`          | 差异块目录：类型短名 + 首行预览；点击跳转；无勾选；滚动不重建条目                                                                                                        |
| `src/composables/chunkKind.ts`              | 差异块类型分类与计数（新增 / 删除 / 修改）；页眉副行文案；滚动不重算                                                                                                     |
...
- `TwoWayMergeEditor`：左参考配置、右目标配置；栏头导入/粘贴全文/清空，整栏拖入走 `importSide`（合法则格式化一次）；空栏轻量提示；编辑器内 Ctrl+V 仍是光标插入、不格式化；`→` 把参考块写入目标配置（`←` 暂时下线；按类型着色，当前锚点块可辨）；按行 diff；差异块目录点击跳转，与缩略轨并存（双侧都空时一并隐藏，滚动不重算冲突带与目录预览）；Ctrl+F 打开编辑器外查找条；上一个差异/下一个差异按视口与块像素带重叠锚点步进并绕回（滚动不重测每块 `lineBlockAt`）；页眉当前块随视口滚动；文档或块数变化时 `chunks` 带上类型构成，滚动不重分类
