...
- **Merge 双栏**：左参考配置、右目标配置，始终左右并排；中间 revert 槽 `→` 按块类型着色并标出当前块（`←` 写回参考暂时下线）；宿主可 `overflow-x: auto`；缩略轨左侧为差异块目录（类型 + 首行预览，点击跳转），右侧冲突点缩略轨（双侧都空时一并隐藏）；空栏轻量提示 + 整栏落区；查找条在栏头与编辑器之间，不覆盖代码
- **窄屏**：仍左右并排并可横滑；**禁止** `.cm-mergeViewEditors { flex-direction: column }`（纵向堆叠会对错 revert 行）
- **页眉 = 全宽半透明层**（透 canvas，不用白色 surface；轻模糊分层），不吸顶、不高卡片；首行品牌 +「本地处理」悬停说明 + `ThemeToggle`；次行差异徽章（无差异 / 当前序号，有差异时副行新增 / 删除 / 修改构成）、「上一个差异 / 下一个差异」、查找、复制、导出菜单（原文 / 压缩）
