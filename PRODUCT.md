...
纯前端的 **可编辑双栏文本合并工作台**：导入两侧 JSON 文本 → 满高 MergeView 对照与编辑 → 中间 `→` 将参考块写入结果 → 复制或下载右栏（默认 `config.json`）。成功标准是主路径可走通、用户 JSON 默认不出域。
...
- 已具备：Merge 栏头点选/粘贴全文/清空，整栏拖入文件（合法 object/array 根则格式化一次）、分栏空态提醒与虚线落区、可编辑 MergeView（左参考、右结果）、按行差异块、冲突点缩略轨（双侧都空时隐藏）、栏头外查找条、块级 `→`（采纳参考）、上一条/下一条（从视口锚点步进，当前 / 总数，绕回）、复制/下载/压缩下载右栏（非法 JSON 提示后仍导出）、亮/暗主题切换
...
- 现行规格与计划：`.docs/specs/2026-08-18-*`、`.docs/specs/2026-08-19-empty-drop-bidirectional-revert.md`、`.docs/specs/2026-08-19-revert-unlock-chunk-anchor.md`、`.docs/specs/2026-08-19-chunk-nav-scroll-perf.md`、`.docs/plans/archive/2026-08-18-*`、`.docs/plans/2026-08-19-empty-drop-bidirectional-revert.md`、`.docs/plans/2026-08-19-revert-unlock-chunk-anchor.md`
